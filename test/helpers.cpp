...
comparison_result_t recursivefilter_and_compare_gpuvscpu(const CpuTable& input,
	...
	summed_area_table_in_vec.emplace_back(CpuTable(input.num_rows(), input.num_cols()));
	const float runtime_1kernelrun_ms = recursivefilter_downright_gpu(
			input, filter_coeff_0, filter_coeff_1, tableblockdim_x,
			tableblockdim_y, num_kernel_runs, output_step, summed_area_table_in_vec);
	...
	comparison_result.equals = ground_truth.equals(summed_area_table_in_vec[0], max_abs_error);
