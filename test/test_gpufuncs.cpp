...
const float kMaxAuthorsBigTableRunTimeMs = 0.5f;
const float kMaxBigTableRunTimeMs = 0.8f;
const float kMaxHugeTableRunTimeMs = 3.0f;
...
TEST(GPU_funcs, checkmath_step1_sat) {
	...
	try {
		const float runtime_1kernelrun_ms = recursivefilter_downright_gpu(
				...
				kBlockSizeSmallX, kBlockSizeSmallY, 1, OUTPUT_STEP::STEP_1,
				step1_outputs);
	...
	const bool output0_equals = step1_outputs[0].equals(ground_truth_0, kMaxAbsError);
	EXPECT_TRUE(output0_equals);
	const bool output1_equals = step1_outputs[1].equals(ground_truth_1, kMaxAbsError);
	EXPECT_TRUE(output1_equals);
}

TEST(GPU_funcs, checkmath_step2_sat) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
...
TEST(GPU_funcs, checkmath_step3_sat) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const int n_gt_rows = (n_rows / kBlockSizeSmallY + 1), n_gt_cols = (n_cols / kBlockSizeSmallX + 1);
	const int ground_truth_data[] = {
		16,	4,
		20,	6
	};
...
TEST(GPU_funcs, checkmath_step4_sat) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
...
TEST(GPU_funcs, checkmath_step5_sat) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
...
TEST(GPU_funcs, checkmath_step1_arbitrary) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const float ground_truth_0_data[] = {
		3.971520747928f,	4.118169868f,	1.454411868f,	4.398624879928f,	3.030762747928f,
		0.0f,	1.723f,	3.446f,	1.723f,	3.446f
	};
	CpuTable ground_truth_0(n_gt0_rows, n_gt0_cols, &(*ground_truth_0_data));

	const int n_gt1_rows = n_rows, n_gt1_cols = (n_cols / kBlockSizeSmallX + 1);
	const float ground_truth_1_data[] = {
		3.45195298555194f,	2.968729f,
		8.32589494977925f,	7.558384034f,
		14.3575978912594f,	4.126877682564f,
		12.1762270487436f,	5.22200421467995f,
...
TEST(GPU_funcs, checkmath_step2_arbitrary) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const float ground_truth_data[] = {
		3.971520747928f,	4.118169868f,	1.454411868f,	4.398624879928f,	3.030762747928f,
		0.35296155699821f,	2.08899472616396f,	3.57525816331534f,	2.11391964635472f,	3.71535343066228f,
	};
	CpuTable ground_truth(n_gt_rows, n_gt_cols, &(*ground_truth_data));
...
TEST(GPU_funcs, checkmath_step3_arbitrary) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const float ground_truth_data[] = {
		12.1762270487436f,	5.22200421467995f,
		8.17774632887718f,	6.40155396103111f,
	};
	CpuTable ground_truth(n_gt_rows, n_gt_cols, &(*ground_truth_data));
...
TEST(GPU_funcs, checkmath_step4_arbitrary) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const int n_gt_rows = 5, n_gt_cols = (n_cols / kBlockSizeSmallX + 1);
	const float ground_truth_data[] = {
		3.45195298555194,	3.27551493360004,
		8.32589494977925,	8.29833254022669,
		14.3575978912594,	5.40288262578305,
		12.1762270487436,	6.30414386099312,
...
TEST(GPU_funcs, checkmath_step5_arbitrary) {
	const int n_rows = 5, n_cols = 5;
	const int input_data[] = {
		1, 0, 0, 1, 1,
		2, 1, 1, 1, 2,
	...
	const int n_gt_rows = 5, n_gt_cols = 5;
	const float ground_truth_data[] = {
		2.968729f,			1.620926034f,		0.885025614564f,	3.45195298555194f,	4.85349533011136f,
		7.558384034f,		7.095606682564f,	6.84293024867995f,	8.32589494977925f,	12.1043226765795f,
		7.095606682564f,	11.4325852826799f,	10.8318465983433f,	14.3575978912594f,	11.9661261311916f,
		6.84293024867995f,	10.8318465983433f,	8.42013989125942f,	12.1762270487436f,	11.8702241832939f,
		3.73623991577925f,	8.88291724269542f,	12.1557804146276f,	13.743826651178f,	16.2928016527584f,
	};
	CpuTable ground_truth(n_gt_rows, n_gt_cols, &(*ground_truth_data));
	...
	const float runtime_1kernelrun_ms = recursivefilter_downright_gpu(
		input, kArbitratyFilterCoeffs[0], kArbitratyFilterCoeffs[1],
		kBlockSizeSmallX, kBlockSizeSmallY, 1, OUTPUT_STEP::STEP_5_FINAL, outputs);
	Logger::new_line("Step 5 output:");
	Logger::new_line(outputs[0].toString());
	Logger::new_line();
...
TEST(GPU_funcs, checkmath_authorstinytable) {
	...
	const comparison_result_t comparison_result = recursivefilter_and_compare_gpuvscpu(
		input, kArbitratyFilterCoeffs[0], kArbitratyFilterCoeffs[1], kBlockSizeSmallX,
		kBlockSizeSmallY, 1, OUTPUT_STEP::STEP_5_FINAL, kMaxAbsError, true, false);
	ASSERT_TRUE(comparison_result.equals);
}

TEST(GPU_funcs, checkmath_oddnumcols) {
...
TEST(GPU_funcs, checkmath_oddnumrows) {
...
TEST(GPU_funcs, checkmath_oddnumcolsnumrows) {
...
TEST(GPU_funcs, checkmath_authorsbigtable) {
	const int n_rows = kAuthorsBigTableSizeY, n_cols = kAuthorsBigTableSizeX;
	CpuTable input(n_rows, n_cols);	input.setSawTooth(0.14f, 29);
	Logger::new_line("Authors' big table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
	
	const comparison_result_t comparison_result = recursivefilter_and_compare_gpuvscpu(
		input, kArbitratyFilterCoeffs[0], kArbitratyFilterCoeffs[1], kBlockSizeLargeX,
		kBlockSizeLargeY, 1, OUTPUT_STEP::STEP_5_FINAL, kMaxAbsError, false, false);
	ASSERT_TRUE(comparison_result.equals);
}

TEST(GPU_funcs, checkmath_bigtable) {
...
TEST(GPU_funcs, checkmath_hugetable_satfilter_fullonesfill) {
	const int n_rows = kBigTableSizeY, n_cols = kBigTableSizeX;
	CpuTable input(n_rows, n_cols, 1.0f);
	Logger::new_line("Huge table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
	
	const comparison_result_t comparison_result = recursivefilter_and_compare_gpuvscpu(
		input, kSatFilterCoeffs[0], kSatFilterCoeffs[1], kBlockSizeLargeX, kBlockSizeLargeY,
		1, OUTPUT_STEP::STEP_5_FINAL, kMaxAbsError, false, false);
	ASSERT_TRUE(comparison_result.equals);
}

TEST(GPU_funcs, checkmath_hugetable_satfilter_arbitraryfill) {
	const int n_rows = kHugeTableSizeY, n_cols = kHugeTableSizeX;
	CpuTable input(n_rows, n_cols);
	input.setSawTooth(0.18f, 29);
	Logger::new_line("Huge table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
	
	const comparison_result_t comparison_result = recursivefilter_and_compare_gpuvscpu(input,
		kSatFilterCoeffs[0], kSatFilterCoeffs[1], kBlockSizeLargeX, kBlockSizeLargeY,
		1, OUTPUT_STEP::STEP_5_FINAL, kMaxAbsError, false, false);
	ASSERT_TRUE(comparison_result.equals);
}

TEST(GPU_funcs, checkmath_hugetable_arbitraryfilter_arbitraryfill) {
	const int n_rows = kHugeTableSizeY, n_cols = kHugeTableSizeX;
	CpuTable input(n_rows, n_cols);
	input.setSawTooth(0.18f, 29);
	Logger::new_line("Huge table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
	...
	const comparison_result_t comparison_result = recursivefilter_and_compare_gpuvscpu(input,
		kArbitratyFilterCoeffs[0], kArbitratyFilterCoeffs[1], kBlockSizeLargeX, kBlockSizeLargeY,
		1, OUTPUT_STEP::STEP_5_FINAL, kMaxAbsError, false, false);
	ASSERT_TRUE(comparison_result.equals);
}

TEST(GPU_funcs, measuretime_authorsbigtable_arbitraryfilter_arbitraryfill) {
	const int n_rows = kAuthorsBigTableSizeY, n_cols = kAuthorsBigTableSizeX;
	CpuTable input(n_rows, n_cols);
	input.setSawTooth(0.18f, 29);
	...
	const comparison_result_t comparison_result = 
		recursivefilter_and_compare_gpuvscpu(input, kArbitratyFilterCoeffs[0],
			kArbitratyFilterCoeffs[1], kBlockSizeLargeX, kBlockSizeLargeY,
			kNumKernelRunsForMeasuringTime, OUTPUT_STEP::STEP_5_FINAL,
...
TEST(GPU_funcs, measuretime_bigtable_arbitraryfilter_arbitraryfill) {
	const int n_rows = kBigTableSizeY, n_cols = kBigTableSizeX;
	CpuTable input(n_rows, n_cols);
	input.setSawTooth(0.18f, 29);
	Logger::new_line("Big table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
...
TEST(GPU_funcs, measuretime_hugetable_arbitraryfilter_arbitraryfill) {
	const int n_rows = kHugeTableSizeY, n_cols = kHugeTableSizeX;
	CpuTable input(n_rows, n_cols);
	input.setSawTooth(0.18f, 29);
	Logger::new_line("Huge table size: (" + std::to_string(n_cols) + ", " + std::to_string(n_rows) + ")");
